...
## 🔎 Codebase Inspection Update (ล่าสุด)

จากการตรวจสอบฐานโค้ดรอบนี้ ได้ดำเนินงานแก้ไขอย่างละ 1 งานตามหมวดหลัก:

- **งานแก้ข้อความพิมพ์ผิด**: แก้ข้อความแสดงผล `BLOCH STATS` เป็น `BLOCK STATS` ในสคริปต์ identity inspector เพื่อให้สอดคล้องกับศัพท์ที่อ่านเข้าใจได้ทันที
- **งานแก้บั๊ก**: ปรับ `test_identity.py` และ `test_tachyon.py` ไม่ให้ `sys.exit(1)` ระหว่าง import เมื่อไม่พบ `tachyon_core` (ลดการพังของ pytest ระหว่าง collection)
- **งานแก้ความคลาดเคลื่อนคอมเมนต์/เอกสารในโค้ด**: ปรับ docstring ของ payload helper ให้ตรงกับ layout จริง และตัดพารามิเตอร์ `turbulence` ที่ไม่ได้ใช้งานจริงออกจากฟังก์ชัน
- **งานปรับปรุงการทดสอบ**: เพิ่ม regression tests เพื่อยืนยันว่า script ทั้งสอง import ได้อย่างปลอดภัยแม้ไม่มี `tachyon_core` และยืนยัน payload layout ขนาด/offset สำคัญ

### แนวทางต่อยอดเชิงสร้างสรรค์

- เพิ่ม CI job แยกสำหรับ “optional native module” โดยรันทดสอบทั้งโหมด `with tachyon_core` และ `without tachyon_core` เพื่อลด false negative
- เพิ่ม static check สำหรับ “unused parameters” และ “import-time side effects” เพื่อกันปัญหาเดิมเกิดซ้ำ
- สำหรับ dedup schema รอบใหม่ แนะนำผูก regression test กับ canonical/quality rubric เดียวกันทุก pipeline (frontend + backend + export)
